...
export class UsersController {
    ...
    @Post(':id/2fa')
    // @UseGuards(JwtAuthGuard)
    switch2fa(@Param('id', ParseIntPipe) id, @Body('status', ParseBoolPipe) status, @Body('code') code: string | undefined, @Res() response) {
        console.log("bounjouern sdf", status)
        let isCodeValid = null
        if (code)
            isCodeValid = this.usersService.isCodeValid(code, id)
        if (!status && !isCodeValid) {
