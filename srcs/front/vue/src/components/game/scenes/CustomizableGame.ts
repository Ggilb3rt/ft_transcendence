...
import ball from "../assets/balls/default/ball.png";
import paddle from "../assets/paddles/default/paddle.png";
...
export default class LevelOneScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("p1", paddle);
    this.load.image("p2", paddle);
  }
  ...
  update() {
    ...
    if (scene.settingsOK && !scene.joinQueue) {
      scene.socket.emit("joinQueue", { level: 2 });
  ...
  createGameObjects(settings, width, height, scene) {
    scene.initBackground(width, height, scene);
    scene.initBallObject(settings, width, height, scene);
    scene.initPlayerObjects(settings, width, height, scene);
  ...
  initBallObject(settings, width, height, scene) {
  ...
  initPlayerObjects(settings, width, height, scene) {
